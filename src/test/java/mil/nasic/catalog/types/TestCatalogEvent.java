...
public class TestCatalogEvent {
    ...
    public void testICRecord() throws Exception {
        ...
        String json = IOUtils.toString(TestCatalogRecord.class.getResourceAsStream("catalogevent.json"));
        EventMessage evM = CatalogJsonUtils.readObject(json, EventMessage.class);
        
        CatalogEvent catEvent = (CatalogEvent) evM.getEvent();

        Assert.assertTrue(catEvent.getCatalog().getMeta().getGeom() instanceof Point);
